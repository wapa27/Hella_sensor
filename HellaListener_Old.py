...
def handle_sensor():
    try:
        ...
        # Extract the IP address from alive_notification
        namespaces = {"sde2": "http://www.aglaia-gmbh.de/xml/2013/05/17/BaSS_SOAPd.xsd"}
        alive_notification = tree.find(".//sde2:alive_notification", namespaces)
        if alive_notification is not None:
            ip_address = alive_notification.get("ip_address")
            if ip_address and ip_address == ip1:
